...
# Intro
This week we focus on checking and dealing with assumptions for linear regression and ANOVA.
We also briefly talk about the `ggplot2` package and learn how to use it to visualise data.
...
For example, when we make the sex variable a factor variable, male = 0, female = 1, 
sex will be displayed as male/female in `R`. However, when it is entered into a regression model, 
...
See how it is now after being recoded using the `table()` function.

```{r}
table(d$Sex)
```

If you view the datset, you will see that the labels are displayed.
...
```{r}
leveneTest(Variable1 ~ Sex, data = d)
```
