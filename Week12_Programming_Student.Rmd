...
Run linear models predicting Distance flown between airports (`distance`) from 
amount of time spent in the air (`air_time`) and `carrier`,
stratified by month of departure. 

These models will have the same set of predictors and outcomes, but different datasets. 
...
In this example, we will loop through each variable in the `flights` dataset, 
check whether it is a character variables and if so, convert to factors.
