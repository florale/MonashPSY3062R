...
To do this, we add `!` before `is.na()`. `!` means NOT. Here, we make a new dataset `d3` that only 
...
`d3` and `d4` are identical using `identical()` function.
...
* Another approach is using `rowMeans()` function. We use `rowMeans()` to get an average score. 
  We then get back to the "sum" scale by multiple the result to the number of items 
