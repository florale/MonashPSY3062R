...
In this example, I used the `simpsons` palette, which is inspried by the coloursused in The Simpsons. 
...
We heve learnt how to improve our `visreg` object, so in this example, let's see how we can 
...
We use the `jco` palette from `ggsci` this time. The `facet_grid()` function lets us 
